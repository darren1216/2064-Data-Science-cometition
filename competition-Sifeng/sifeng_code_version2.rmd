...
training <- train[1:floor(length(train[, 1]) * 0.8),]
testing <- train[(length(training[, 1]) + 1):22,]
```

```{r}
# model 1: Linear Regression
model_1 <- lm(outcome~., data = training)
summary(model_1)
prediction_1 <- predict(model_1, newdata = testing)
RMSE(predict(model_1))
```
...
rmse(log(testing$outcome), log(prediction_2$fit))
```
...
rmse(log(testing$outcome), log(prediction_3))
```